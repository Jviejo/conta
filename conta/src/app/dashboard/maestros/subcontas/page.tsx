...
export default function SubcontasPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      await fetch(`/api/subcontas/${editingId}`, {
        method: 'PUT',
  ...
  const handleEdit = (subconta: Subconta) => {
    setFormData({ id: subconta.id, nombre: subconta.nombre });
    setEditingId(subconta._id?.toString() || null);
    setShowForm(true);
  };

  const handleDelete = async (id: string) => {
    if (confirm('¿Estás seguro de eliminar esta subconta?')) {
      ...
      {showForm && (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow mb-6">
          <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-4">
            {editingId ? 'Editar Subconta' : 'Nueva Subconta'}
          </h2>
            ...
            {subcontas.map((subconta) => (
              <tr key={subconta._id?.toString()}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  <button
                    onClick={() => handleEdit(subconta)}
                    className="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 dark:hover:text-indigo-300 mr-4"
                  >
                    Editar
                  </button>
                  <button
                    onClick={() => handleDelete(subconta._id?.toString() || '')}
