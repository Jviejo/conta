...
export default function NivelesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      await fetch(`/api/niveles/${editingId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(formData),
      });
    } else {
      await fetch('/api/niveles', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(formData),
      });
  ...
  const handleEdit = (nivel: Nivel) => {
    setFormData({ id: nivel.id, nombre: nivel.nombre });
    setEditingId(nivel._id?.toString() || null);
    setShowForm(true);
  };

  const handleDelete = async (id: string) => {
    if (confirm('¿Estás seguro de eliminar este nivel?')) {
      await fetch(`/api/niveles/${id}`, { method: 'DELETE' });
      fetchNiveles();
    }
  };
            ...
            {niveles.map((nivel) => (
              <tr key={nivel._id?.toString()}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  ...
                  <button
                    onClick={() => handleDelete(nivel._id?.toString() || '')}
