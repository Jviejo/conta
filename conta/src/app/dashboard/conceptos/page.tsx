...
export default function ConceptosPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      await fetch(`/api/conceptos/${editingId}`, {
        method: 'PUT',
  ...
  const handleEdit = (concepto: Concepto) => {
    setFormData({ id: concepto.id, nombre: concepto.nombre });
    setEditingId(concepto._id?.toString() || null);
    setShowForm(true);
  };
            ...
            {conceptos.map((concepto) => (
              <tr key={concepto._id?.toString()}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  ...
                  <button
                    onClick={() => handleDelete(concepto._id?.toString() || '')}
